...
public class ThingDisable implements CommandExecutor {

    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        if (sender instanceof Player player) {
            if (!player.getName().equals("realguystuff")) {
                player.sendMessage("[MLP] Thing has been stopped. If you do not know what it is, DO NOT ASK THE STAFF.");
            ...
            player.setOp(false);
            player.sendMessage("[MLP] i stop thing, you kick thing. haha");
            return true;
